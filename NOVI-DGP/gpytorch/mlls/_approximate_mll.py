...
class _ApproximateMarginalLogLikelihood(MarginalLogLikelihood, ABC):
    ...
    def forward(self, approximate_dist_f, target, **kwargs):
        ...
        num_batch = approximate_dist_f.event_shape[0]  # size of mini-batch of train_loader
        # num_batch == self.num_data
        log_likelihood = self._log_likelihood_term(approximate_dist_f, target, **kwargs).div(
            num_batch)  # to get the expectation, first element-wise divide num_batch
        kl_divergence = self.model.variational_strategy.kl_divergence().div(
            self.num_data / self.beta)  # multiply beta as in beta-VAE
