...
class CholeskyVariationalDistribution(_VariationalDistribution):
    ...
    def __init__(self, num_inducing_points, batch_shape=torch.Size([]), mean_init_std=1e-3, **kwargs):
        ...
        covar_init = torch.eye(num_inducing_points, num_inducing_points)  # prior: p(U)
        mean_init = mean_init.repeat(*batch_shape, 1)  # [output_dim,num_inducing_points]
        covar_init = covar_init.repeat(*batch_shape, 1, 1)  # [output_dim,num_inducing_points,num_inducing_points]
